...
public class Rei extends Peca {
	...
	public ArrayList<Posicao> criaListaDestinosPossiveis(Tabuleiro tabuleiro, ArrayList<Posicao> todasPossibilidadesInimigo) {
		...
		while (!achou) { // Só verifica se achou o outro rei por que nao é possivel um tabuleiro sem os dois reis
			if ((tabuleiro.getPecas().get(i).getTipo() == EPeca.REI) && (tabuleiro.getPecas().get(i).getCor().compareTo(this.getCor()) != 0)) {
		...
		while (i < posicoes.size()) {
			...
			while ((j < todasPossibilidadesInimigo.size()) && (!retirou)) {
				if ((posicoes.get(i).getX() == todasPossibilidadesInimigo.get(j).getX()) && (posicoes.get(i).getY() == todasPossibilidadesInimigo.get(j).getY())) { // verifica se nesta posição alguma peça poderia pegar o Rei
					posicoes.remove(i);
					retirou = true;
					i--; 
