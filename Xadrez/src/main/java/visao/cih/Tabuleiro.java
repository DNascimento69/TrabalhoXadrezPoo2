...
public class Tabuleiro extends JPanel {
	...
	public Tabuleiro(Validacao validador) {
		...
		pecasBrancas = new ArrayList<Peca>();
		pecasPretas = new ArrayList<Peca>();
	...
	private void inicializarPecas() {
		...
		pecasBrancas.add(new Peca(2, true));
		pecasBrancas.add(new Peca(2, true));
		pecasBrancas.add(new Peca(3, true));
		pecasBrancas.add(new Peca(3, true));
		pecasBrancas.add(new Peca(4, true));
		pecasBrancas.add(new Peca(4, true));
		pecasBrancas.add(new Peca(5, true));
		pecasBrancas.add(new Peca(6, true));
	...
	private void inicializarCasas() {
		...
		while (x < 8) {
			while (y < 8) {
				matrizTabuleiro[x][y] = new Casa(
						casasStartX + (y * arestaCasa), casasStartY
								+ (x * arestaCasa), x, y, this);
	...
	public boolean moverPeca(Casa casa1, Casa casa2) {
		...
		if (!validador.validaMovimento(casa1, casa2))
			return false;

		tmp = matrizTabuleiro[casa2.xTabuleiro][casa2.yTabuleiro]
				.setPeca(matrizTabuleiro[casa1.xTabuleiro][casa1.yTabuleiro]
						.removerPeca());
...
/*
 * while(x < 2) { while(y < 8) {
 * matrizTabuleiro[x][y].setPeca(pecasBrancas.get(count1)); y++; count1++; }
 * 
 * y = 0;
 * 
 * while(y < 8) { matrizTabuleiro[x + 6][y].setPeca(pecasPretas.get(count2));
 * y++; count2++; }
 * 
 * y = 0; x++; }
 */
...
/*
 * public boolean moverPeca(int x, int y, int x2, int y2) { if(x < 0 || x > 7 ||
 * y < 0 || y > 7 || x2 < 0 || x2 > 7 || y2 < 0 || y2 > 7) return false;
 * 
 * 
