...
public class StrategyMoveDiagonalCima implements StrategyMoveInterface {
	private ELimiteCasas limitacaoCasas;
	
	public StrategyMoveDiagonalCima() {}
	...
	public List<String> movePool(Tabuleiro tabuleiro, Peca peca) {
		...
		while((x <= this.limitacaoCasas.getLimiteCasas()) && (!parar) && (linhaAuxiliar <= 7) && (colunaAuxiliar <= 7)) {
		...
		while((x <= this.limitacaoCasas.getLimiteCasas()) && (!parar) && (linhaAuxiliar <= 7) && (colunaAuxiliar >= 0)) {
